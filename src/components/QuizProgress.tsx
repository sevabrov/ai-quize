...
export function QuizChecklist({ currentIndex, visible = 8, className }: ChecklistProps) {
  const total = questions.length
  const passed = currentIndex
  const percent = Math.round((passed / total) * 100)

  // Вікно навколо поточного питання, щоб список не «тікав» уперед
  const start = Math.min(Math.max(0, currentIndex - 3), Math.max(0, total - visible))
  const window = questions.slice(start, start + visible)
  const rest = total - (start + window.length)

  return (
    <div className={cn('rounded-panel border border-line bg-white p-6 shadow-card', className)}>
      ...
      <div className="mt-2 flex items-center gap-3">
        ...
        <span className="font-display text-xs font-extrabold text-leaf-600">{percent}%</span>
      </div>
