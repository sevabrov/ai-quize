...
### Чому в проді немає бейджа «Локально»

Vite читає `.env` і під час прод-збірки, тому `VITE_API_URL=http://localhost:3001`
раніше потрапляв у бандл: у браузері відвідувача кожна відповідь давала
провалений fetch до неіснуючого localhost і бейдж «Локально».

Тепер `env.ts` вважає localhost-адресу ознакою локальної розробки: у прод-збірці
вона означає статичний режим (`apiUrl = ''`). Індикатор синхронізації
рендериться тільки коли `isApiEnabled` - тобто лише на localhost. Реальний
віддалений бекенд у `VITE_API_URL` при цьому працюватиме у проді як звичайно.
